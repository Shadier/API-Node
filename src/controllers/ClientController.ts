...
clientRouter.get('/', (req, res) => {
	Client.find((err, clients) =>{
		...
		if(!clients) return res.status(404).send({message: 'clients not founded!'})
...
clientRouter.get('/:id', (req, res) => {
	...
	Client.findById(clientId, (err, client) =>{
		if(err) return res.status(500).send({message: 'Internal Server error'})
		if(!client) return res.status(404).send({message: 'Client not founded!'})
...
clientRouter.post('/', (req, res) => {
	...
	if (params.name && params.lastname && params.email) {
		...
		Client.find({email: client.email.toLowerCase()}).exec((err, clients) => {
			...
			if(clients && clients.length >= 1) return res.status(200).send({message: "email registered before, please use another"})
			else{
...
clientRouter.delete('/:id', (req, res) => {
	...
	Client.findByIdAndRemove(clientId, (err, clientDeleted) => {
		...
		if(clientDeleted) res.status(200).send({messafe: 'Client Deleted successfully!'})
		else res.status(404).send({message: 'Client not Deleted!'})
	})
